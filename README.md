...
torch-tensorrt>=1.0.0  # Optional for TensorRT support
```
...
```bash
python Composition/Codes/simple_test.py --test_path test_data_path --result_path result_save_path --pretrained_path model_path --gpu 0
```

## Single Image Pair Testing

```bash
python Composition/Codes/simple_test.py --single_test --warp1 image1_path --warp2 image2_path --mask1 mask1_path --mask2 mask2_path --output_dir output_directory --pretrained_path model_path
```

## Simplified Single Image Pair Testing

If you only need to perform simple testing for the Composition process without loading complex diffusion models, you can use the following lightweight script:

```bash
python Composition/Codes/test_composition.py --warp1 image1_path --warp2 image2_path --mask1 mask1_path --mask2 mask2_path --output_dir output_directory
```

This simplified script uses a simple linear blending method to compose images, suitable for quickly validating data and masks. It provides the following outputs:

1. **Input images**: `input1.jpg` and `input2.jpg`
2. **Mask images**: `mask1.jpg` and `mask2.jpg`
3. **Composition result**: `composition.jpg`
4. **Visualization result**: `visualization.png` - comparison image containing all input and output images
5. **Evaluation metrics**: `metrics.txt` - objective evaluation metrics including PSNR and SSIM

The script automatically handles single-channel and multi-channel masks, ensuring correct composition. The entire process doesn't depend on GPU, runs quickly, and is suitable for fast debugging and result preview.

### Use Case

Testing a single image pair from the dataset:
```bash
...
This project now supports single-machine multi-GPU and multi-machine training based on PyTorch Distributed Data Parallel (DDP). Use the following command to start distributed training:

```bash
# Recommended method: using torchrun script (more reliable)
...
1. The `batch_size` parameter represents the batch size per GPU, total batch size = `batch_size` × number of GPUs
2. Ensure each GPU has sufficient memory; if OOM errors occur, try reducing `batch_size` or enabling gradient accumulation
3. Distributed training uses the NCCL backend, optimized for NVIDIA GPUs
4. Only the rank 0 process will save models, record logs, and perform validation
5. Synchronized BatchNorm can be enabled with the `--sync_bn` parameter to improve multi-GPU training stability

## Single GPU Training

If you only have a single GPU or don't want to use distributed training, you can use the following command:

```bash
python main.py --mode train --part composition --data_dir /path/to/data
```

## More Detailed Distributed Training Instructions

Please refer to [DISTRIBUTED_TRAINING.md](DISTRIBUTED_TRAINING.md) for more detailed distributed training instructions, including parameter details, multi-machine training configuration, and troubleshooting guide.
